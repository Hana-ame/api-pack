...
import (
	...
	"github.com/Hana-ame/api-pack/internal/exhentai/common"
	myfetch "github.com/Hana-ame/api-pack/pkg/fetch/v2"
...
func ExhProxy(client *myfetch.Client, addr string) {
	godotenv.Load(".env")

	// 1. 初始化网络与 Fetcher
	p := &ProxyHandler{client}
...
func (p *ProxyHandler) accessControlMiddleware() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		// 26.07.06
		// 【坑】c.Redirect 后必须 c.Abort(), 否则 mainProxyHandler 继续执行覆盖响应,
		// 拦截失效 (同 exhentai/exhentai.go 的修复, 2026-08-25 E2E 发现)
...
// 核心转发逻辑 - 从 env 读取 endpoint，保留 HTML 内容修改
func (p *ProxyHandler) doProxy(c *gin.Context) {
	endpoint := tools.Or(os.Getenv("EXHENTAI_ENDPOINT"), "exhentai.org")
	targetURL := "https://" + endpoint + c.Request.URL.String()

	reqHeader := p.prepareHeaders(c)

	resp, err := p.client.Fetch(c.Request.Method, targetURL, reqHeader, c.Request.Body)
	if err != nil {
...
func (p *ProxyHandler) handleSpecialRedirects(c *gin.Context, data []byte) bool {
	...
	switch redir {
	case "image":
		if img, err := common.FindOneAndSelectAttr(doc, "//img[@id='img']", "src"); err == nil {
			c.Redirect(http.StatusFound, img)
			return true
		}
	case "cover":
		hrefs := common.FindAll(doc, "//a", "href")
		for _, h := range hrefs {
